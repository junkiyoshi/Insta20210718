		...
		for (int i = 0; i < word.size(); i++) {
			...
			for (int outline_index = 0; outline_index < outline.size(); outline_index++) {
				...
				for (auto& vertex : vertices) {
					...
					auto angle_x = ofMap(ofNoise(noise_param.x, noise_location.x * 0.5, ofGetFrameNum() * 0.01), 0, 1, -PI * 1.5, PI * 1.5);
					auto angle_y = ofMap(ofNoise(noise_param.x, noise_location.y * 0.5, ofGetFrameNum() * 0.01), 0, 1, -PI * 1.5, PI * 1.5);
					auto angle_z = ofMap(ofNoise(noise_param.x, noise_location.z * 0.5, ofGetFrameNum() * 0.01), 0, 1, -PI * 1.5, PI * 1.5);
